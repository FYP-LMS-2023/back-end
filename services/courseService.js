...
exports.createCourse = async (req, res, next) => {
  ...
  if (!program)
    return res.status(400).send({ message: "Program does not exist!" });
...
exports.getAllCourses = async (req, res, next) => {
  ...
  for (var i = 0; i < courses.length; i++) {
    const program = await Program.find({ cores: `${courses[i]._id}` }).select(
      "-faculty -electives -description"
    );
    // courses[i] = { ...courses[i], program: program[0] };
    // courses[i] = x;
    let x = { course: courses[i], program: program };
    courses[i] = x
  }
  if (!courses) {
